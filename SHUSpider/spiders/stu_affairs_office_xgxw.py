...
class ShunewsSpider(scrapy.Spider):
    name = 'stu_affairs_office_xgxw'
    allowed_domains = ['xgb.shu.edu.cn']
    start_urls = ["http://www.xgb.shu.edu.cn/Default.aspx?tabid=31641"]
    browser = None
    next_page = None
    ...
    def parse(self, response):
        ...
        while response:
            post_nodes = response.css("#dnn_ctr59828_ArticleList__ctl0_ArtDataList a::attr(href)").extract()
            news_time = response.css("#dnn_ctr59828_ArticleList__ctl0_ArtDataList__ctl0_Label6::text").extract_first()
            ...
            self.browser.find_element_by_css_selector("#dnn_ctr59828_ArticleList__ctl0_lbtnNext").click()
            selector = Selector(text=self.browser.page_source)
            page_num = selector.css("#dnn_ctr59828_ArticleList__ctl0_plPageNum::text").extract_first()
    ...
    def parse_detail(self, response):
        ...
        # 部门
        item_loader.add_css("apartment", "#dnn_ctr59828_ArticleDetails_ctl00_hypDept::text")
        # 发布人
        author = response.css("#dnn_ctr59828_ArtDetail_hypFirst::text").extract_first() + response.css(
